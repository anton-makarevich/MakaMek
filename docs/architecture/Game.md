...
### 4. Game Phases
- **Phases:**
  - The game progresses through a set of well-defined phases: Start → Deployment → Initiative → Movement → WeaponsAttack → WeaponAttackResolution → Heat → End. A PhysicalAttack phase is planned between weapon resolution and heat; see below.
  ...
  - **PhysicalAttackPhase:** present in the codebase but **not yet wired into `PhaseManager`**, so it does not run. Planned to sit between weapon resolution and heat, supporting adjacent BattleMech punch/kick declarations with authoritative resolution, result broadcast, and pass handling. Push, Charge, DFA, and physical-weapon attacks remain separate follow-up rule slices.
