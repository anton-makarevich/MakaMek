...
```csharp
// All endpoints require the static API key in the X-Api-Key HTTP header.
// Ready, Close, and member deletion additionally require a session token
// (issued by the join or create endpoint) to authenticate the caller.

public record CreateRoomRequest(string PlayerName, Guid PlayerId);
...
public record JoinRequest(string PlayerName, Guid PlayerId);
...
// Auth: X-Api-Key header + session token in request body or Authorization header
public record ReadyRequest(string SessionToken);
public record ReadyResult(bool Success, string? ErrorMessage);

// Auth: X-Api-Key header + session token in request body or Authorization header
public record CloseRequest(string SessionToken);
...
**Room state** is the same as specified below (Created → Active → Closed → Dissolved), but transitions happen via REST rather than SignalR.
...
|---|---|---|
| **Created** | `POST /api/rooms` succeeds | Room exists; host not yet running `ServerGame`. Other clients cannot join until host is ready. |
| **Active** | `POST /api/rooms/{roomCode}/ready` succeeds | Accepting clients; game commands relayed between all members. |
| **Closed** | `POST /api/rooms/{roomCode}/close` succeeds | Game in progress; new players rejected with `RoomFull`. Known players (playerId in roster) may rejoin for reconnection. |
...
**TTL:** Rooms expire after 2 hours of inactivity. This applies to all states, including `Created` — a room whose host never calls `POST /api/rooms/{roomCode}/ready` is garbage-collected after 2 h. In-memory only — no database.
...
public interface IRelayHub
{
    ...
    // The hub validates that the caller's session (from OnConnectedAsync) is a
    // member of the specified roomCode before fan-out. A mismatched or
    // non-member roomCode is rejected with an error; messages are only relayed
...
public class RelayHub : Hub<IRelayHub>
{
    ...
    {
        // Validate that the caller belongs to the specified room.
        var callerRoom = _roomManager.GetPlayerRoom(Context.ConnectionId);
        if (callerRoom != roomCode)
...
public class RelayClientPublisher : ITransportPublisher, IAsyncDisposable
{
    ...
    private void HandleEnvelopeReceived(RelayEnvelope envelope)
    {
        try
        {
            ...
            if (message == null)
            {
                // Log and ignore null deserialization result
                return;
            }
        ...
        {
            // Malformed payload — log the error and ignore.
            // The per-connection Relay() rate limit mitigates repeated abuse.
            _logger.LogWarning(ex, "Failed to deserialize relay envelope from {SenderId}", envelope.SenderId);
        }
    }
...
- **New: static shared API key.** A single key baked into official clients, required to call REST endpoints and connect to the hub. For REST calls, the key is transmitted via the `X-Api-Key` HTTP header. For WebSocket connections (where browsers cannot set custom headers), it travels as a query-string parameter on the hub URL and is checked before the WS upgrade completes. This is **explicitly not real protection** — it's extractable from any client build and doesn't identify individual users — its only job is filtering out low-effort automated/opportunistic traffic hitting a public endpoint.
- **Session tokens** issued by the REST join endpoint serve as the hub's authentication mechanism. The hub validates the token via middleware before `OnConnectedAsync` fires — no room-level RPC authentication is needed. Session tokens are short-lived (tied to the room's TTL) and revocable (invalidated when the room dissolves or the player is removed).
- **Credential leakage controls.** Both session tokens and the static API key travel in query strings on the WebSocket URL (browsers cannot set custom headers during WS handshake). To limit credential exposure: proxies, load balancers, access logs, tracing systems, and diagnostic toolkits **must** redact query-string parameters when logging or recording connection URLs. Full connection URLs (including query strings) must never be logged at any layer. The same redaction applies to the `X-Api-Key` header value in REST access logs.
...
| Credential leakage | Session tokens and the API key travel in query strings (WS) or headers (REST). Proxies, access logs, tracing, and diagnostics must redact query-string parameters and sensitive header values. Full connection URLs must never be logged. Session tokens are short-lived (room TTL) and revocable (room dissolution / member removal). |
...
- Implement `RoomManager` service — room lifecycle (Created → Active → Closed → Dissolved), 6-char base32 codes, 2 h TTL, player roster.
...
- Unit-test `HandleEnvelopeReceived` with an invalid/malformed payload: verify it logs the error and does not interrupt subsequent message processing.
