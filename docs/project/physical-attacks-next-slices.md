...
The first production change should introduce the declaration/path contract before adding the enum
value to `PhysicalAttackType`.
...
`PhysicalAttackType` deliberately declares only `Punch` and `Kick`. Push, Charge, and DFA are not
representable in the command contract, so no partially-implemented attack type can reach the
validator or resolver. Each new attack type should be added to the enum only once its rules slice
above is complete.
