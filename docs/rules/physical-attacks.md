...
| **Charge** | **Movement Phase** | `+0` *(± Piloting diff.)* | **Target:** (Attacker Tonnage / 10) × Hexes Moved *(round up)*.<br>**Attacker:** Target Tonnage / 10 *(round up)*. | Grouped in 5-point clusters. **Hexes Moved** counts only hexes traversed *before* entering the target's hex. Displaces target; forces **both** units to make a PSR (+2 modifier) to avoid falling. No weapon attacks allowed. |
| **Death From Above (DFA)** | **Movement Phase** | `+0` *(± Piloting diff. + Jump mod)* | **Target:** (Attacker Tonnage / 10) × 3 *(round up)*.<br>**Attacker:** Attacker Tonnage / 5 *(round up)* to legs. | Grouped in 5-point clusters on **Punch Location Table** for target. Attacker lands in target hex, target is displaced. Both make PSRs (Target +2, Attacker +4). No weapon attacks allowed. |
