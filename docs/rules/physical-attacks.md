...
| **Push** | Physical Phase | `-1` | `0` *(No damage)* | Pushes target 'Mech 1 hex away and forces target to make a PSR or fall. Attacker advances into target's former hex. Requires both arms. |
| **Physical Weapon** | Physical Phase | Varies *(e.g., Hatchet -1, Sword -2)* | Varies *(e.g., Hatchet = Tonnage / 5; Sword = Tonnage / 10 + 1)* | Uses **Standard Hit Location Table** (or Punch/Kick table with a +4 modifier). Cannot fire weapons on the weapon arm. |
| **Charge** | **Movement Phase** | `+0` *(± Piloting diff.)* | **Target:** (Attacker Tonnage / 10) × Hexes Moved.<br>**Attacker:** Target Tonnage / 10. | Grouped in 5-point clusters. Displaces target; forces **both** units to make a PSR (+2 modifier) to avoid falling. No weapon attacks allowed. |
| **Death From Above (DFA)** | **Movement Phase** | `+0` *(± Piloting diff. + Jump mod)* | **Target:** (Attacker Tonnage / 10) × 3.<br>**Attacker:** Attacker Tonnage / 5 to legs. | Grouped in 5-point clusters on **Punch Location Table** for target. Attacker lands in target hex, target is displaced. Both make PSRs (Target +2, Attacker +4). No weapon attacks allowed. |

---

### **4. Other Units & Special Conditions**
