...
public class BattleMap(int width, int height, string biome = "makamek.biomes.grasslands") : IBattleMap
{
    ...
    private IMovementCostProvider _movementCostProvider = DefaultMovementCostProvider;

    /// <summary>
    /// Gets or sets the terrain movement-cost rules used by pathfinding.
    /// Changing the provider invalidates cached movement paths.
    /// </summary>
    public IMovementCostProvider MovementCostProvider
    {
        get => _movementCostProvider;
        set
        {
            ArgumentNullException.ThrowIfNull(value);
            if (ReferenceEquals(_movementCostProvider, value)) return;
            _movementCostProvider = value;
            _movementPathCache.Clear();
            _movementLongPathCache.Clear();
        }
    }
    ...
    /// <param name="fromSurface">Surface level of an attacker unit, bull if no attacker (check from the lowest level)</param>
    /// <param name="toSurface">Surface level of a target unit, null if no unit in a target hex</param>
    public LineOfSightResult GetLineOfSight(HexCoordinates from, HexCoordinates to, int attackHeight, int targetHeight = 0, HexSurface? fromSurface = null, HexSurface? toSurface = null)
    {
        ...
        // Compute submersion and standing levels using the surface-aware helper
        var (attackerStandingLevel, attackerSubmerged) = fromHex.GetLosEndpointInfo(fromSurface, attackHeight);
        var (targetStandingLevel, targetSubmerged) = toHex.GetLosEndpointInfo(toSurface, targetHeight);
        var attackerLosLevel = attackerStandingLevel + attackHeight;
        var targetLosLevel = targetStandingLevel + targetHeight;
        
        if (attackerSubmerged != targetSubmerged)
        {
            return LineOfSightResult.Blocked(
                from: from, to: to,
                blockingHex: attackerSubmerged ? from : to,
        ...
        if (from == to)
            return LineOfSightResult.Unblocked(from: from, to: to,
                attackerLosLevel: attackerLosLevel, targetLosLevel: targetLosLevel,
        ...
        {
            Hex = toHex,
            InterpolatedLosHeight = targetLosLevel,
            InterveningFactor = toHex.GetCeiling() >= targetLosLevel
    ...
    private MovementPath? FindJumpPath(HexPosition from, HexPosition to, int movementPoints)
    {
        ...
        var cachedPath = _movementPathCache.Get(from, to, true, unitHeight: 0);
        if (cachedPath != null)
        {
