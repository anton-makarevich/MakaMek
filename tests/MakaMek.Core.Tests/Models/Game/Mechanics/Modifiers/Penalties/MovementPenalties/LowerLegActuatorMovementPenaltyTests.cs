...
public class LowerLegActuatorMovementPenaltyTests
{
    ...
    {
        // Arrange
        var sut = new LowerLegActuatorMovementPenalty
        {
            DestroyedCount = 2,
            Value = 2
        };
        _localizationService.GetString("Penalty_LowerLegActuatorMovement").Returns("Lower Leg Actuator Movement Penalty ({0} destroyed): -{1} MP");

        // Act
        var result = sut.Render(_localizationService);

        // Assert
        result.ShouldBe("Lower Leg Actuator Movement Penalty (2 destroyed): -2 MP");
        _localizationService.Received(1).GetString("Penalty_LowerLegActuatorMovement");
    }
