...
public class MovementStateTests
{
    ...
    public void ResumeMovementAfterFall_ShouldLockMovementType_WhenMpSufficient()
    {
        ...
        var position = new HexPosition(new HexCoordinates(1, 2), HexDirection.Bottom);
        var proneMech = _battleMapViewModel.Units.First() as Mech;
        _pilotingSkillCalculator.GetPsrBreakdown(proneMech!, Arg.Any<PilotingSkillRollContext>())
        ...
        var targetHex = _battleMapViewModel.Game!.BattleMap!.GetHex(new HexCoordinates(1, 3))!;
        _sut.HandleHexSelection(targetHex);
        _sut.HandleFacingSelection(HexDirection.Bottom); 
        ...
        // Assert
        var actions = _sut.GetAvailableActions().ToList();
        var standupAction = actions.FirstOrDefault(a => a.Label.Contains("Run"));
        standupAction.ShouldNotBeNull();
        actions.Any(a => a.Label.Contains("Walk")).ShouldBeFalse();
    }

    [Fact]
    public void ResumeMovementAfterFall_ShouldCompleteMovement_WhenMpInsufficient()
    {
        // Arrange
        var position = new HexPosition(new HexCoordinates(1, 2), HexDirection.Bottom);
        var proneMech = _battleMapViewModel.Units.First() as Mech;
        ...
        // Move unit so it has < 2 MP left. Distance 4 takes 4 MP. Walk MP is usually 4 for dummy mech.
        var targetHex = _battleMapViewModel.Game!.BattleMap!.GetHex(new HexCoordinates(1, 6))!;
        _sut.HandleHexSelection(targetHex);
        _sut.HandleFacingSelection(HexDirection.Bottom); 
