...
public class EndGamePlayerViewModelTests
{
    ...
    {
        // Arrange
        var player = new Player(Guid.NewGuid(), "TestPlayer", PlayerControlType.Human, "#FF0000");
        var mech = CreateMech();
        player.AddUnit(mech);
    ...
    public void Units_ShouldPassLocalizationServiceToUnitCards()
    {
        ...
        _localizationService.GetString("UnitItem_NoPilot").Returns("Sin Piloto");

        // Act
        var sut = new EndGamePlayerViewModel(player, isVictor: false, _localizationService);

        // Assert
        sut.Units[0].CardViewModel.PilotName.ShouldBe("Sin Piloto");
    }
    ...
    {
        // Arrange
        var player = new Player(Guid.NewGuid(), "VictoriousPlayer", PlayerControlType.Human);
    ...
    {
        // Arrange
        var player = new Player(Guid.NewGuid(), "TestPlayer", PlayerControlType.Human, "#00FF00");
